 ...
 [0, 900] ]; // Closing the top of the king
	...
	*/
	function Build3DSurfaceFrom2DCurve(rz, scale, powdivs, nnubs, knots, domain2)
	{
		...
		if (domain2 == null) domain2 = DOMAIN([[0,1],[0,1]])([rz.length*3,nnubs]); 
...
if (onlyboard == false)
{
	...
	var whiteKing = COLOR([1,1,1])( king );
	var blackKing = COLOR([0,0,0])( king );

	var pawn = T([0,1])([bes/2,bes/2])(Build3DSurfaceFrom2DCurve(rzPawn, scalePawn));
	var whitePawn = COLOR([1,1,1])( pawn );
	var blackPawn = COLOR([0,0,0])( pawn );
}
